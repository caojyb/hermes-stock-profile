...
if str(_STOCK_WORK) not in sys.path:
    from core.bootstrap import ensure_stock_work_root as _ensure
    _ensure()
...
def get_holdings():
    ...
    if _sim_path:
      try:
        conn = sqlite3.connect(_sim_path, timeout=60)
        cur = conn.cursor()
        cur.execute("""
...
RECOMMEND_TRACK_DIR = str(Path(__file__).resolve().parent.parent.parent / 'stock-work' / 'data' / 'outputs' / 'intraday')
...
def track_to_pool(alerts, source="intraday_scan"):
    ...
    conn = sqlite3.connect(POOL_DB, timeout=60)
    cur = conn.cursor()
