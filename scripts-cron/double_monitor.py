...
from core.compat_paths import INTRADAY_CACHE_DB
from stock_db_paths import get_db_path
MARKET_DB = str(get_db_path('market_cache'))
SIM_DB = str(get_db_path('simulation'))
SIM_TEST_DB = str(get_db_path('simulation_test')) if os.getenv('SIM_MODE', 'production').strip().lower() == 'test' else None
RUN_ID = str(__import__('uuid').uuid4())[:8]  # 每次运行唯一标识，用于 portfolio_snapshots 审计
# P2-2: 模拟仓交易标记策略来源
...
# ── pending 信号失效检查（3 个交易日，当前按自然日简化） ──
# TODO: 替换为交易日历，当前为自然日简化实现
def _check_pending_signals_expiry(conn, cur, max_trade_date):
    """检查并失效过期的 pending 信号"""
    try:
        # 从 klines 获取最近交易日
        cur.execute("SELECT MAX(trade_date) FROM klines WHERE trade_date <= ?", (max_trade_date,))
        latest_trade_date = cur.fetchone()[0]
        
        if not latest_trade_date:
            return
        
        # 计算 3 个自然日前的日期（TODO: 换交易日历）
        expiry_date = (datetime.strptime(latest_trade_date, '%Y-%m-%d').date() - timedelta(days=3)).isoformat()
        
        # 失效过期的 pending 信号
        cur.execute("""
            UPDATE signals 
            SET status='expired', reason='超过 3 个交易日未执行', updated_at=?
            WHERE status='pending' AND signal_date < ?
        """, (max_trade_date, expiry_date))
        
        expired_count = cur.rowcount
        if expired_count > 0:
            print(f"  ⚠️ 失效 {expired_count} 个过期 pending 信号（截止 {expiry_date}）")
    except Exception as e:
        print(f"  ⚠️ pending 失效检查失败: {e}")
...
""")
def validate_portfolio_snapshot(today_str, today_total, sim_cur):
    """校验 portfolio_snapshots 日环比合理性。
    返回 (ok: bool, msg: str)。
    """
    try:
        row = sim_cur.execute(
            "SELECT date, total_value FROM portfolio_snapshots WHERE date < ? ORDER BY date DESC LIMIT 1",
            (today_str,),
        ).fetchone()
    except Exception as e:
        return True, f"昨日快照查询失败，跳过环比校验: {e}"

    if row is None or row[1] is None or row[1] <= 0:
        return True, "无昨日快照，跳过环比校验"

    yesterday_total = float(row[1])
    if yesterday_total <= 0:
        return True, "昨日 total_value 非正，跳过环比校验"

    change_pct = (today_total - yesterday_total) / yesterday_total
    if change_pct > 0.5:
        return False, f"日环比异常 +{change_pct*100:.1f}%（昨日 {yesterday_total:,.0f} -> 今日 {today_total:,.0f}），拒绝写入"
    return True, "环比校验通过"
...
for code, h in list(open_map.items()):
    ...
    if ret <= -STOP_LOSS:
        ...
        try:
            ...
            from decision_alert_counter import increment
            increment('double_monitor_sell', _e)
...
if new_entry_ok and current_count < 20 and IS_TRADING_DAY:
    ...
    # 检查并失效过期的 pending 信号
    _check_pending_signals_expiry(sim_conn, sim_cur, today_str)
    sim_conn.commit()  # 显式提交失效检查结果
    ...
    for stock in buy_candidates[:max_new]:
        ...
        try:
            ...
            from decision_alert_counter import increment
            increment('double_monitor_buy', _e)
...
try:
    ...
    ok, snap_msg = validate_portfolio_snapshot(today_str, tv, sim_cur)
    if not ok:
        print(f"   [WARN] {snap_msg}")
        # 拒绝写入，但继续执行后续摘要输出
    else:
        sim_cur.execute(
            "INSERT INTO portfolio_snapshots (date, total_value, cash, holdings_value, "
            "total_return_pct, max_drawdown_pct, win_count, loss_count, written_by, run_id) "
            "VALUES (?,?,?,?,?,?,?,?,?,?)",
            (today_str, round(tv, 2), round(cash, 2), round(holdings_value, 2),
             round(trp, 2), round(drawdown, 2), win_cnt, loss_cnt, 'double_monitor', RUN_ID))
        sim_conn.commit()

    # 本地输出落盘（no_agent 可观测性）
    try:
        import json as _json, os as _os
        _out_dir = _os.path.expanduser('~/.hermes/profiles/stock/cron/output/db39df50d53e')
...
try:
    ...
    write(
        'double-monitor-daily',
        'ok',
        detail=f'completed|{len(WATCH_LIST)} stocks',
