...
NORTH_LOG = "/home/caojy/.hermes/profiles/stock/scripts/cron/north_flow_log.json"

# ── Data Provider Layer: FundFlowProvider ──
_NORTH_FLOW_PROVIDER_METRICS = {
    "provider_used": 0,
    "fallback_count": 0,
...
def _init_fund_flow_provider():
    try:
        ...
        from core.data_provider.fund_flow.eastmoney import EastMoneyFundFlowProvider
        from core.data_provider.health import ProviderHealth

        manager = ProviderManager()
        health = ProviderHealth(provider="eastmoney_fund_flow_north_monitor")
        provider = EastMoneyFundFlowProvider(health=health)
        manager.register_provider("eastmoney_fund_flow", provider)
        return manager, provider
    except Exception:
        return None, None

_NORTH_FLOW_MANAGER, _NORTH_FLOW_PROVIDER = _init_fund_flow_provider()
...
def fetch_north_top(limit=50):
    ...
    if _NORTH_FLOW_PROVIDER is not None:
        try:
            ...
            fallback_count = 0
            try:
                ranked = _NORTH_FLOW_PROVIDER.get_northbound_rank(limit=limit, descending=True)
                buy_top = [{'code': r.code, 'name': r.name, 'net_buy': r.net_buy, 'hold_value': r.hold_value} for r in ranked]
            except Exception:
                fallback_count += 1
            try:
                ranked_sell = _NORTH_FLOW_PROVIDER.get_northbound_rank(limit=limit, descending=False)
                sell_top = [{'code': r.code, 'name': r.name, 'net_buy': r.net_buy, 'hold_value': r.hold_value} for r in ranked_sell]
            except Exception:
                fallback_count += 1

            provider_latency_ms = (time.perf_counter() - start) * 1000

            if buy_top or sell_top:
                _NORTH_FLOW_PROVIDER_METRICS["provider_used"] += 1
                _NORTH_FLOW_PROVIDER_METRICS["provider_latency_ms"] += provider_latency_ms
                _NORTH_FLOW_PROVIDER_METRICS["fallback_count"] += fallback_count
                return buy_top, sell_top
        except Exception:
            pass

    # Fallback to legacy
    _NORTH_FLOW_PROVIDER_METRICS["fallback_count"] += 1
    start = time.perf_counter()
    buy_top, sell_top = _legacy_fetch_north_top(limit)
    legacy_latency_ms = (time.perf_counter() - start) * 1000
    _NORTH_FLOW_PROVIDER_METRICS["legacy_latency_ms"] += legacy_latency_ms
    return buy_top, sell_top
...
def cross_check_with_pool(candidates, buy_top, sell_top):
    ...
    for s in candidates:
        ...
        if code in buy_map:
            b = buy_map[code]
            buy_matches.append({'code': code, 'name': s.get('name', '') if isinstance(s, dict) else code,
                'net_buy': b['net_buy'], 'hold_value': b['hold_value']})
        if code in sell_map:
            sl = sell_map[code]
            sell_matches.append({'code': code, 'name': s.get('name', '') if isinstance(s, dict) else code,
