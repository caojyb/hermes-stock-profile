...
from core.compat_paths import MARKET_DB as MKT_DB

FEISHU_DIR = Path("/home/caojy/.hermes/profiles/stock/skills/stock/stock-expert/skills/feishu-bitable")
...
def check_risk_layer():
    ...
    try:
        ...
        if not os.path.exists(sim_path):
            ...
            conn = sqlite3.connect(sim_path, timeout=60)
            cur = conn.cursor()
            cur.execute("""
...
def check_execution_layer():
    ...
    try:
        if os.path.exists(str(SIM_DB)):
            conn = sqlite3.connect(str(SIM_DB), timeout=60)
            cur = conn.cursor()
