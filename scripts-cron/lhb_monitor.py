...
CACHE_DB = str(get_db_path('lhb_cache'))
...
def _prev_trade_date() -> str:
    ...
    """
    try:
        con = sqlite3.connect(_STOCK_MARKET_DB, timeout=60)
...
if __name__ == '__main__':
    ...
    _t0 = time.time()
    result = run(trade_date=_prev_trade_date(), weekly=False)

    # 心跳：记录本次龙虎榜扫描完成
    try:
        from heartbeat import write
        _lhb_count = result.get('total', 0) if isinstance(result, dict) else 0
        write(
            'stock-lhb-daily',
            'ok',
            detail=f'completed|{_lhb_count} 条',
