...
from core.compat_paths import INTRADAY_CACHE_DB as _STOCK_INTRADAY_CACHE_DB
...
def run():
    ...
    print()
    print(f'   ✅ 完成')
    # P1-4 修复（2026-09-21）：补心跳写入
    # 此前本脚本一行心跳代码都没有，而唯一写 'stock-intraday-minute' 心跳的
    # intraday_monitor.py:602 cron 并不运行 → health_check 永远报该任务过期，告警无法自愈
    try:
        import sys as _sys
        _hb_dir = '/home/caojy/.hermes/profiles/stock/scripts/cron'
        if _hb_dir not in _sys.path:
            _sys.path.insert(0, _hb_dir)
        from heartbeat import write as _hb_write
        _hb_write('stock-intraday-minute', 'ok',
                  detail=f'cache cycle complete: {n} minute rows, {n2} 5min bars',
                  expected_interval_seconds=900)
    except Exception as _e:
        print(f"[EXC] intraday_cache.heartbeat: {type(_e).__name__}: {_e}")
