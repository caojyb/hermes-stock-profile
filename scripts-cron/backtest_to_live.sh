...
FEISHU_DIR="$HOME/.hermes/profiles/stock/skills/stock/stock-expert/skills/feishu-bitable"
OUTPUT_DIR="$HOME/.hermes/profiles/stock/cron/output"
...
for i in 1 2 3; do
    ...
    cd "$FEISHU_DIR" && /home/caojy/.hermes/profiles/stock/.venv/bin/python3 backtest_to_live.py \
        --apply \
        --start "$START" \
        --end "$END" \
...
if [ -f "$OUTPUT_DIR/backtest_live_applied.json" ]; then
    BEST_STRATEGY=$(/home/caojy/.hermes/profiles/stock/.venv/bin/python3 -c "
import json
