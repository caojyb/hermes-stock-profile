...
def main():
    ...
    # ── Data Provider Layer shadow adapter (read-only) ──
    try:
        from core.data_provider.adapter.realtime_adapter import RealtimeShadowAdapter
        adapter = RealtimeShadowAdapter(shadow_mode=True)
        sample_codes = [row[0] for row in get_stocks()[:20]]
        if sample_codes:
            shadow_results = adapter.compare_batch(sample_codes)
            summary = adapter.summary()
            print(f'[DATA_PROVIDER] shadow adapter: sampled={summary.get("total", 0)}, matched={summary.get("matched", 0)}, provider_success_rate={summary.get("provider_success_rate", 0):.1f}%, legacy_success_rate={summary.get("legacy_success_rate", 0):.1f}%, avg_price_diff={summary.get("avg_price_diff", 0):.4f}')
    except Exception as e:
        print(f'[DATA_PROVIDER] shadow adapter skipped: {e}')
    ...
    try:
        ...
        write(
            ...
            detail='completed' if not errors else '; '.join(errors),
