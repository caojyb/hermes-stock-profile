...
from core.compat_paths import REAL_PORTFOLIO_HISTORY_DB
...
# Baseline Real Portfolio Truth Contract: 唯一 Canonical Source
CANONICAL_REAL_PORTFOLIO_SOURCE = f"{REAL_HOLDINGS_SOURCE}:{REAL_HOLDINGS_BASE}/{REAL_HOLDINGS_TABLE}"
...
def _derive_quality_status(data_quality: str, freshness: str) -> str:
    """Baseline Real Portfolio Truth Contract: top-level quality_status derivation."""
    if data_quality in ('MISSING', 'INVALID'):
        return data_quality
    if freshness in (EXPIRED, 'EXPIRED'):
        return 'STALE'
    if data_quality == 'STALE':
        return 'STALE'
    if data_quality in ('VALID', 'PARTIAL', 'WARNING'):
        return data_quality
    return data_quality or UNKNOWN
...
# 默认历史序列路径
_DEFAULT_HISTORY_DB = REAL_PORTFOLIO_HISTORY_DB
...
def build_real_snapshot(
    ...
    # P0 修复（2026-09-18）：质量告警必须浮出到 stdout，不允许只静默写入 snapshot JSON
    if qr.get('error_count', 0) > 0 or qr.get('warning_count', 0) > 0:
        print(f"🚨 REAL_PORTFOLIO_QUALITY: overall={qr.get('overall')} "
              f"errors={qr.get('error_count', 0)} warnings={qr.get('warning_count', 0)}")
        for _f in qr.get('flags', [])[:20]:
            print(f"   [{_f.get('level')}] {_f.get('symbol')} {_f.get('name')} "
                  f"{_f.get('field')}: {_f.get('reason')} {_f.get('detail') or ''}")
    ...
    if source_timestamp:
        try:
            ...
            print(f"[EXC] real_portfolio_truth.py: {type(_e).__name__}: {_e}")
            freshness = UNKNOWN
    else:
        freshness = UNKNOWN
    ...
    portfolio = {
        ...
        # 兼容别名：调用方（position_stop_loss_alert._portfolio_context 等）历史键名为 total_holdings_value
    ...
    return {
        ...
        'quality_status': _derive_quality_status(data_quality, freshness),
...
def snapshot_portfolio_context(snap: dict) -> dict:
    ...
    return {
        ...
        # 兼容历史键名 total_holdings_value（portfolio dict 实际键为 holdings_value）
        'total_holdings_value': p.get('total_holdings_value') or p.get('holdings_value') or 0,
