...
class StopLossChainTest(unittest.TestCase):
    ...
    def test_no_executed_outcome_from_safe_run(self):
        # 2026-09-19: 旧路径 SCRIPT_DIR/simulation.db 已迁移至 canonical
        # stock-work/data/runtime/simulation.db（compat_paths 单点解析）
        from core.compat_paths import SIMULATION_DB
        conn = sqlite3.connect(f"file:{SIMULATION_DB}?mode=ro", uri=True)
