...
class TestSignalFinalSeparation(unittest.TestCase):
    ...
    def test_opportunity_output_is_signal_wording(self):
        ...
        f = Path('/home/caojy/.hermes/profiles/stock/cron/output/1aa2fd36bdef/2026-08-26_15-31-33.md')
        if not f.exists():
            self.skipTest('样本不存在')
    ...
    def test_stop_loss_output_carries_decision_id(self):
        f = Path('/home/caojy/.hermes/profiles/stock/cron/output/21540a83af1b/2026-08-26_09-35-58.md')
        if not f.exists():
            self.skipTest('样本不存在')
        txt = f.read_text(encoding='utf-8')
    ...
    def test_deep_review_has_no_decision_id(self):
        ...
        f = Path('/home/caojy/.hermes/profiles/stock/cron/output/e4a2c0461481/2026-08-26_16-05-53.md')
        if not f.exists():
            self.skipTest('样本不存在')
        import re
...
class TestActionability(unittest.TestCase):
    ...
    def test_daily_report_sell_items_have_reason_and_id(self):
        # 2026-09-19: 8/25 报告已随 9-11 清理; 改验现役最近一份日报
        import glob as _glob
        _reps = sorted(_glob.glob(str(Path(SCRIPT_DIR, 'reports/daily_decision_*.json'))))
        if not _reps:
            self.skipTest('无现役日报产物')
        r = json.load(open(_reps[-1]))
...
class TestConflictClassification(unittest.TestCase):
    ...
    def test_same_day_multi_action_is_display_only(self):
        ...
        # 2026-09-19: 原锚点 split('else:') 命中的是远处 else（误报）。
        # 缩小到 MULTI_ACTION 段本体（buy_codes → multi_note 之间）。
        seg = src.split('buy_codes = {t[0] for t in buys}')[1].split('multi_note =')[0]
        self.assertNotIn('UPDATE', seg, 'MULTI_ACTION 分支不得写 trades')
        self.assertNotIn('INSERT INTO trades', seg, 'MULTI_ACTION 分支不得写 trades')
        conn = ro_conn()
...
class TestIsolationAndHoldings(unittest.TestCase):
    ...
    def test_simulation_trades_never_marked_production(self):
        # 2026-09-19: 09-18 重置后 trades=0（旧账归档 trades_legacy_202607）。
        # 现役表必须无"非 v1_double 策略"/"已执行实盘"行; 归档表豁免（历史留痕）。
        conn = ro_conn()
...
class TestDeterminism(unittest.TestCase):
    ...
    def test_cron_inventory_matches_registry(self):
        ...
        # 2026-09-19: 现役 24 job 命名演进, 旧前缀只 match 11。
        # 本 jobs.json 即 stock profile 全部 cron（无混住）, enabled 全量即股票 job。
        stock_jobs = [j for j in lst if j.get('enabled', True)]
