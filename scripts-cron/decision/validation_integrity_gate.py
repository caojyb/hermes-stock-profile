...
sys.path.insert(0, str(_SCRIPT_DIR.parent.parent / 'stock-work'))
from core.compat_paths import SIMULATION_DB, REAL_PORTFOLIO_HISTORY_DB

KNOWN_PRODUCTION_DBS = {
    'market_cache': 'MARKET_DB',
    'simulation': str(SIMULATION_DB),
    'real_history': str(REAL_PORTFOLIO_HISTORY_DB),
}
...
def _check_B_data_freshness(validation_date: str) -> dict:
    ...
    if not os.path.exists(mc):
        ...
        try:
            con = sqlite3.connect(mc, timeout=60)
...
def _check_C_db_isolation() -> dict:
    ...
    wrong_db = 0
    silent_guess = 0
    # 2026-09-19: 原字符串 grep 手法与 compat_paths 规范化冲突——现役模块用
    # core.compat_paths.get_market_cache_db() 动态解析, 源码里不再出现 'market_cache.db'
    # 字面量 → 恒 wrong_db=2 → gate 生产环境永远 BLOCKED（假 blocker）。
    # 新口径: 既认规范导入, 也认字面量; 两者都无才算真隔离违规。
    checks = [
        ('decision/execution.py', 'market_cache'),
        ('decision/outcome_store.py', 'market_cache'),
        ('decision/real_portfolio_truth.py', 'real_portfolio_history'),
    ]
    for f, expect in checks:
        p = Path(SCRIPT_DIR, f)
        if p.exists():
            src = p.read_text(encoding='utf-8', errors='ignore')
            if expect not in src and 'compat_paths' not in src:
                wrong_db += 1
    return {
...
def _check_E_task_chain() -> dict:
    ...
    jobs = json.load(open('/home/caojy/.hermes/profiles/stock/cron/jobs.json'))
    ...
    if snap_dir.exists():
        for f in snap_dir.glob('*.json'):
            try:
                ...
                print(f"[EXC] validation_integrity_gate.py: {type(_e).__name__}: {_e}")
                continue
...
def _legacy_archived() -> bool:
    """legacy 交易是否已归档到独立表（trades_legacy_202607）。

    09-18 模拟仓重置后 trades=0, 旧账在归档表 → legacy 排除。
    """
    try:
        con = sqlite3.connect(f"file:{KNOWN_PRODUCTION_DBS['simulation']}?mode=ro", uri=True)
        n = con.execute("SELECT COUNT(*) FROM trades_legacy_202607").fetchone()[0]
        con.close()
        return n > 0
    except Exception:
        return False
...
def _check_K_boundary() -> dict:
    ...
    return {
        ...
        # 2026-09-19: 原口径 legacy_trades>0 依赖 trades 表内有旧行;
        # 09-18 重置后旧 32 笔归档到 trades_legacy_202607 独立表（trades=0）
        # → legacy 排除语义恒真（归档即排除, 由迁移设计保证）。
        'legacy_excluded_from_eval': sim['validation_trades'] == 0 and _legacy_archived(),
