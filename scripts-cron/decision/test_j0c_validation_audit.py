...
# 2026-09-19: canonical simulation.db 已迁至 stock-work/data/runtime/
# （09-18 模拟仓重置: trades=0, 旧 32 笔归档 trades_legacy_202607）;
# 本文件是 8/24 对旧库快照的审计留痕测试, 断言改为"现役表 + 归档表"双口径。
try:
...
# 2026-09-19: 本文件是 8/24 对"旧模拟库"的审计留痕测试。09-18 重置后:
#   - trades=0（旧 32 笔 → trades_legacy_202607）
#   - portfolio_snapshots 只剩 9-18/9-19 基线（旧 8 月快照 → portfolio_snapshots_legacy_202607）
# 剩余 TestContaminationClassification/GateClassification 类断言 8 月 NAV bug 取证
# （648585.52 虚低、>200万虚高等）——断言对象已归档, 现役库无此数据。
# 正确处置=显式 skip（保留源码作历史文档, 不用假数据糊弄）; 上面两个类已改为
# legacy 归档表口径继续提供真实回归价值。
LEGACY_SNAPSHOTS_GONE = True  # 8 月快照已归档, 现役 portfolio_snapshots 无 <2026-09 行
...
class TestPeriodSplit(unittest.TestCase):
    ...
    def test_pre_validation_rows_exist(self):
        # 2026-09-19: PRE_FIX_LEGACY 基准行已随重置归档到
        # portfolio_snapshots_legacy_202607（数据安全网, 23 行）; 现役表按设计
        # 只含 validation 期（9-18 起的 100 万基线）。
        conn = ro_conn()
        n = conn.execute("SELECT COUNT(*) FROM portfolio_snapshots_legacy_202607 WHERE date < ?",
                         (VALIDATION_START,)).fetchone()[0]
        conn.close()
        self.assertGreaterEqual(n, 1, 'PRE_FIX_LEGACY 基准行必须保留（归档表）')

    def test_validation_rows_split_deterministic(self):
        conn = ro_conn()
        # 2026-09-19: 口径改"归档表+现役表"合并后仍满足 pre+val=total（分期不丢行）
        pre = conn.execute("SELECT COUNT(*) FROM portfolio_snapshots_legacy_202607 WHERE date < ?", (VALIDATION_START,)).fetchone()[0]
        val = conn.execute("SELECT COUNT(*) FROM portfolio_snapshots_legacy_202607 WHERE date >= ?", (VALIDATION_START,)).fetchone()[0]
        val += conn.execute("SELECT COUNT(*) FROM portfolio_snapshots WHERE date >= ?", (VALIDATION_START,)).fetchone()[0]
        total = conn.execute("SELECT COUNT(*) FROM portfolio_snapshots_legacy_202607").fetchone()[0]
...
class TestNoMutation(unittest.TestCase):
    ...
    def test_readonly_open(self):
        conn = ro_conn()
        n_arch = conn.execute("SELECT COUNT(*) FROM trades_legacy_202607").fetchone()[0]
        conn.close()
        self.assertEqual(n_arch, 32, '旧 32 笔必须完整归档在 trades_legacy_202607')

    def test_trades_facts_intact(self):
        conn = ro_conn()
        buys = conn.execute("SELECT COUNT(*) FROM trades_legacy_202607 WHERE buy_date >= ?", (VALIDATION_START,)).fetchone()[0]
        sells = conn.execute("SELECT COUNT(*) FROM trades_legacy_202607 WHERE sell_date >= ?", (VALIDATION_START,)).fetchone()[0]
        conn.close()
        self.assertEqual(buys, 2)
        self.assertEqual(sells, 22)


@unittest.skipUnless(not LEGACY_SNAPSHOTS_GONE, "legacy 快照审计: 8月NAV数据已归档 portfolio_snapshots_legacy_202607, 现役库无断言对象")
