...
运行：
  cd /home/caojy/.hermes/profiles/stock/scripts/cron && python3 -m pytest decision/test_production_observation_phase8b.py -v
"""
import json, os, glob
...
# 2026-09-19 P0-fix: 原 autouse fixture 直接删生产 snapshots/ 全量 json
# （跑一次测试 = 清空 166 个生产决策快照, 含 9/18 的 50 个）。
# snapshot_verify 的 canonical 写入方是生产链, 测试清理必须走隔离路径。
def _clean_snapshots(snap_dir=None):
    d = snap_dir or snapshot_mod.SNAP_DIR
...
def _clean(tmp_path, monkeypatch):
    ...
    # 隔离 snapshot 写入目标（与 conftest.isolate_snapshots 同手法）
    import decision.daily_decision_contract as _ddc
    import decision.observation as _obs
    monkeypatch.setattr(snapshot_mod, 'SNAP_DIR', str(snap_dir))
    monkeypatch.setattr(_ddc, 'SNAP_DIR', str(snap_dir))
    monkeypatch.setattr(_obs, 'SNAP_DIR', str(snap_dir))
    _clean_executions()
    _clean_outcomes()
    _clean_snapshots(snap_dir)
    yield
    _clean_executions()
    _clean_outcomes()
    _clean_snapshots(snap_dir)
...
def test_04_decision_counts():
    ...
    # 2026-09-19 P0-fix: 原硬编码 Path('decision/snapshots') 相对 cwd 写生产快照目录
    # （不经 SNAP_DIR, monkeypatch 隔离失效 → p8b_* 泄漏进生产 snapshots/）。
    # 改走被 fixture 隔离的 snapshot_mod.SNAP_DIR（= tmp_path/snapshots）。
    snap_dir = Path(snapshot_mod.SNAP_DIR)
...
def test_08_no_trade_observation():
    ...
    # 2026-09-19 P0-fix: 同 test_04 —— 硬编码相对路径写生产目录, 改走隔离 SNAP_DIR
    snap_dir = Path(snapshot_mod.SNAP_DIR)
    snap_dir.mkdir(exist_ok=True)
