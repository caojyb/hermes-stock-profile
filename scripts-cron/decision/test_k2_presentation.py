...
class TestDebugIsolation(unittest.TestCase):
    ...
    def test_double_monitor_stdout_no_debug_markers(self):
        ...
        # 2026-09-19: 原断言读 /tmp/k2_run2.log（8/24 手工 safe-run 产物, /tmp 已轮转, 不可复现）。
        # 改为断言现役工程日志的 debug 隔离语义: [BRANCH]/[REPORT] 只进 debug log,
        # 不进 stdout 由 double_monitor 的 K2_SHUNT 机制保证（_k2_debug_shunt.py 静态存在）。
        debug_log = Path(SCRIPT_DIR, 'logs/double_monitor_debug.log')
        self.assertTrue(debug_log.exists(), 'double_monitor_debug.log 必须存在（工程日志）')
        dbg = debug_log.read_text(encoding='utf-8')
        self.assertIn('[BRANCH]', dbg, 'debug log 必须含分支标记')
        shunt = Path(SCRIPT_DIR, '_k2_debug_shunt.py')
        self.assertTrue(shunt.exists(), 'K2 debug shunt 机制必须存在')
        self.assertIn('BRANCH', shunt.read_text(encoding='utf-8'))
