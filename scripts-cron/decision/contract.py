...
REASON = {
    ...
    'COOLDOWN': 'COOLDOWN',
    # tradability / portfolio truth / evidence
    'TRADABILITY_BLOCKED': 'TRADABILITY_BLOCKED',
    'TRADABILITY_UNKNOWN': 'TRADABILITY_UNKNOWN',
    'PORTFOLIO_TRUTH_INCOMPLETE': 'PORTFOLIO_TRUTH_INCOMPLETE',
...
class Decision:
    ...
    # Baseline Decision State Contract
    policy_version: str = ''     # DecisionEngine 传入的 policy_version，与 engine 对齐
    policy_action: str = ''      # Investment Policy 原始建议
    engine_result: str = ''      # ALLOWED / BLOCKED / NOT_EXECUTABLE
    final_action: str = ''       # 引擎仲裁后的最终动作
    block_reason: str = ''       # 主 block reason
    execution_status: str = ''   # READY / USER_INPUT_REQUIRED / NOT_READY

    # constraint registry trace
    constraint_id: str = ''      # 触发的主 constraint_id
    all_triggered_constraints: list = field(default_factory=list)
