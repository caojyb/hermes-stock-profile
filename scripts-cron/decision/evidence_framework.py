...
# P0 compat: expose core.compat_paths when imported from decision package
_STOCK_WORK = Path(__file__).resolve().parent.parent.parent.parent / 'stock-work'
import sys as _sys
if str(_STOCK_WORK) not in _sys.path:
    _sys.path.insert(0, str(_STOCK_WORK))
...
def _load_json(path: str) -> Dict[str, Any]:
    try:
        ...
        print(f"[EXC] evidence_framework.py: {type(_e).__name__}: {_e}")
        return {}
...
def build_review_record_from_outcome(outcome: Union[Dict[str, Any], Any], ex: Optional[Dict[str, Any]] = None, decision: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # 2026-09-19: review.update(evidence_completeness(review)) 会把 ec dict 展平,
    # 键 'evidence_completeness' 被自身覆盖成字符串（原键名与 ec 内键撞名）。
    # 调用方 test/consumer 取 review['evidence_completeness']['missing_sections'] 得
    # TypeError。改为挂到独立键 'completeness', 顶层字符串键保留兼容。
    _ec = evidence_completeness(review)
    review['completeness'] = _ec
    review['evidence_completeness'] = _ec['evidence_completeness']
    return review
