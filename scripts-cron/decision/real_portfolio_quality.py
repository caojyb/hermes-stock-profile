...
def _check_holding_quality(holding: dict) -> dict:
    ...
    if avg_cost is None:
        ...
        # ratio check
        # 2026-09-18 三轮: 用户确认成本值真实（反映实际盈亏，如用友 +3151%），ratio 大
        # 不是数据错误。降回 WARNING 级提示（不拦截），仅 quantity<=0 等硬错误保留 ERROR。
        if current_price is not None and current_price > 0:
            ratio = avg_cost / current_price
            if ratio > 2:
                checks.append({'field': 'avg_cost', 'level': 'WARNING', 'reason': 'OUTLIER',
                               'detail': f'avg_cost={avg_cost}, current_price={current_price}, ratio={ratio:.2f} '
                                         f'(成本/现价比大——若为真实盈亏则正常，若疑似未复权请在Bitable核对)'})
            elif ratio < 0.5:
...
if __name__ == '__main__':
    import sys
    path = sys.argv[1] if len(sys.argv) > 1 else '/home/caojy/.hermes/profiles/stock/scripts/cron/decision/test_real_holdings_h2.py'
