...
class DecisionEngine:
    ...
    def __init__(self, *, config_version='', code_version='', strategy='v1_double', strategy_version='', policy_version=''):
    ...
    def decide(self, ctx: dict) -> Decision:
        ...
        d = Decision(
            ...
            policy_action=ctx.get('policy_action', ctx.get('entry_signal', ENTRY_NONE)),
            engine_result='',
            final_action='',
            block_reason='',
            execution_status='',
            constraint_id=ctx.get('constraint_id', ''),
            all_triggered_constraints=list(ctx.get('all_triggered_constraints', []) or []),
        )
        ...
        # 2. Tradability Gate（Hard Gate）
        tradability_available = ctx.get('tradability_available')
        tradability_value = ctx.get('tradability') or {}
        liquidity_ok = tradability_value.get('value', {}).get('liquidity_ok') if isinstance(tradability_value, dict) else None
        tradability_status = ctx.get('tradability_status') or tradability_value.get('availability') if isinstance(tradability_value, dict) else None
        if tradability_status in ('UNAVAILABLE', 'UNKNOWN', 'STALE') or liquidity_ok is False:
            d.action = NO_TRADE
            reason = 'TRADABILITY_UNKNOWN' if tradability_status == 'UNKNOWN' else 'TRADABILITY_BLOCKED'
            d.reason_codes.append(REASON.get(reason, reason))
            d.explanation = f'交易可行性未知/不可用({tradability_status})，禁止开新仓'
            return self._finalize(d)
        d.reason_codes.append(REASON.get('TRADABILITY_ALLOWED', 'TRADABILITY_ALLOWED'))

        # 3. Portfolio Truth Gate（Hard Gate）
        portfolio_truth = ctx.get('portfolio_truth') or {}
        cash = portfolio_truth.get('cash')
        total_asset = portfolio_truth.get('total_asset')
        if cash is None or total_asset is None:
            d.action = NO_TRADE
            d.reason_codes.append(REASON['PORTFOLIO_TRUTH_INCOMPLETE'])
            d.explanation = 'Portfolio Truth 不完整（cash/total_asset UNKNOWN），禁止开新仓'
            return self._finalize(d)

        # 4. Portfolio Risk（回撤/暴露/单股/行业/流动性/冷却）—— Phase 3 前置否决
        pa = ctx.get('portfolio_assessment') or {}
    ...
    def _finalize(self, d: Decision) -> Decision:
        # Policy binding: ensure decision carries the engine's policy_version
        if not d.policy_version:
            d.policy_version = self.policy_version
        elif self.policy_version and d.policy_version != self.policy_version:
            d.policy_version = self.policy_version
        
        # Decision State Contract normalization
        d.policy_action = d.policy_action or d.action
        if d.action == NO_TRADE and d.reason_codes:
            d.engine_result = 'BLOCKED'
            d.block_reason = d.reason_codes[0]
            d.execution_status = 'NOT_READY'
        elif d.action in (BUY, ADD, REDUCE, SELL):
            d.engine_result = 'ALLOWED'
            d.execution_status = 'READY'
        else:
            d.engine_result = 'NOT_EXECUTABLE'
            d.execution_status = 'USER_INPUT_REQUIRED'
...
# ---------------------------------------------------------------------------
# Rollback Contract（Baseline §50）：Code + Policy 成对回滚
# ---------------------------------------------------------------------------

def record_rollback(*, target_config_version='', target_code_version='', target_strategy='',
                    target_strategy_version='', reason='', rollback_type='POLICY') -> Dict:
    """记录一次回滚决策，供人工确认和审计。不触发真实交易。"""
    from datetime import datetime, timezone
    return {
        'rollback_id': gen_decision_id('ROLLBACK'),
