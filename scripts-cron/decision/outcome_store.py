...
import sys
# P0 compat: decision 包内导入 core.compat_paths 需显式把 stock-work 加入 sys.path
...
def _conn():
    return sqlite3.connect(MARKET_DB, timeout=60)
...
def build_from_decision(decision, symbol='', action='', regime=''):
    ...
    # Baseline Attribution Contract v1：Decision 层先落 skeleton，Execution/Outcome 生成后再补全
    from .outcome import build_attribution
    o.attribution = build_attribution(decision, {}, {})
    return o
...
def load_all():
    ...
    for f in glob.glob(str(_OUTCOME_DIR / '*.json')):
        try:
            ...
            print(f"[EXC] outcome_store.py: {type(_e).__name__}: {_e}")
            pass
    return outs
