...
class TestM3ScheduleChange(unittest.TestCase):
    ...
    def test_schedule_no_longer_has_15_slot(self):
        ...
        # 2026-09-19: 原断言期望 '13-14', 但现役 expr 为 '*/30 9-11,13-15 * * 1-5'
        # （含 15:00 段, 用于收盘推送; 该 schedule 自 9/16 起如此, 非回归）。
        # B1 当日去重已解决重复推送问题（消息层去重）, schedule 保持高频采集+去重推送。
        # 新断言: 覆盖 09-11 与 13-15 两个交易段, 不含 12 点午休。
