...
def _counts_from_files(dir_path: str, status_key: str) -> dict:
    ...
    for fp in glob.glob(os.path.join(dir_path, '*.json')):
        try:
            ...
            print(f"[EXC] observation.py: {type(_e).__name__}: {_e}")
            continue
        s = d.get(status_key)
        if not s:
            continue
...
# 2026-09-19: 原三处硬编码 Path(__file__).parent/'snapshots' 使测试无法隔离
# （跑一次 observation 测试 = 扫生产 165 个快照）。提取为模块级 SNAP_DIR,
# 默认值保持生产路径不变, 测试可 monkeypatch。
