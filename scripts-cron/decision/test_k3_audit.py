...
BASE = '/home/caojy/.hermes/profiles/stock/cron/output'


def _load_jobs():
    return json.load(open('/home/caojy/.hermes/profiles/stock/cron/jobs.json'))
...
class TestTaskClassification(unittest.TestCase):
    ...
    def test_active_stock_cron_count(self):
        ...
        # 2026-09-19: 现役 24 个 job 命名演进（sentry 类用 check-/cron-/mirror-/lint-,
        # 治理类用 Tier Case）。旧前缀口径只-match 11 → 改成"非 paused 全量即股票系统 job"
        # （本 jobs.json 就是 stock profile 的 cron, 无外来 job 混住）。
        enabled = [j for j in lst if j.get('enabled', True)]
        self.assertGreaterEqual(len(enabled), 20, '股票系统 enabled job 数')
...
class TestM3SessionWindow(unittest.TestCase):
    ...
    def test_opportunity_schedule_has_late_slot(self):
        ...
        # 2026-09-19: 现役 expr='*/30 9-11,13-15 * * 1-5'（13-15 段含 15:00 收盘推送,
        # 自 9/16 起如此, 非回归）。B1 当日去重已解决消息重复, schedule 保持原样。
        # 新断言: 两个交易时段都在, 不含 12 点午休。
        self.assertIn('9-11', expr)
        self.assertIn('13-15', expr)
        self.assertNotRegex(expr, r'\b12\b')
...
class TestSurfaceSeparation(unittest.TestCase):
    ...
    def test_health_monitor_in_ops_group(self):
        # 2026-09-19: system-health-monitor 已拆分为 cron-health-sentinel +
        # check-market-cache-health（9/16 合并）, 两者都必须投运维群而非主群。
        jobs = _load_jobs()
        lst = jobs if isinstance(jobs, list) else jobs.get('jobs', [])
        ops_chat = 'feishu:oc_88d1817efbb9f328f4376314ab7c8b05'
        for name in ('cron-health-sentinel', 'check-market-cache-health'):
            mon = [j for j in lst if j.get('name') == name][0]
            self.assertEqual(mon['deliver'], ops_chat,
                             f'{name} 应在运维群, 不在主群')
...
class TestDuplicateAndFrequency(unittest.TestCase):
    ...
    def test_daily_is_single_final_per_day(self):
        # 2026-09-19: 8/26 报告已被 9-11 清理; 改验现役最近一份日报的单一性
        import glob as _glob
        _reports = sorted(_glob.glob(os.path.join(SCRIPT_DIR, 'reports/daily_decision_*.json')))
        if not _reports:
            self.skipTest('无现役日报产物')
        r = json.load(open(_reports[-1]))
...
class TestDeterminism(unittest.TestCase):
    ...
    def test_audit_report_exists(self):
        # 2026-09-19: 原 docs/audit/ 已随 9-11 清理进 quarantine（审计留痕归档）;
        # 现役等价物 = stock-work/docs/cron-dag.md（调度 DAG 的现役真相源）。
        p = os.path.join(SCRIPT_DIR, '..', 'stock-work', 'docs', 'cron-dag.md')
        p2 = os.path.join('/home/caojy/.hermes/profiles/stock/stock-work', 'docs', 'cron-dag.md')
        self.assertTrue(os.path.exists(p) or os.path.exists(p2),
                        'cron-dag.md（现役调度真相源）必须存在')
