...
# ═══ Baseline Simulation Execution Model v1 ═══
# Signal at T → Order generated at T → Next valid trading session T+1 → Execution at T+1 Open
# Properties: deterministic, reproducible, auditable, free of look-ahead bias
EXECUTION_MODEL_VERSION = 'sim_exec_v1_t1_open'
EXECUTION_MODEL_NAME = 'Canonical Daily T+1 Open'
EXECUTION_MODEL_RULE = 'T+1_OPEN'
...
class Execution:
    ...
    # Baseline: Simulation Execution Model v1 binding
    execution_model_version: str = EXECUTION_MODEL_VERSION
    execution_model_name: str = EXECUTION_MODEL_NAME
    execution_model_rule: str = EXECUTION_MODEL_RULE
    # Reproducibility / Baseline binding
    code_commit: str = ''
    policy_version: str = ''
    config_version: str = ''
    decision_cycle_version: str = ''
...
def find_execution(decision_id):
    ...
    for f in glob.glob(str(_EXEC_DIR / '*.json')):
        try:
            ...
            print(f"[EXC] {type(_e).__name__}: {_e} (execution.py 治理: 静默吞异常已改为可见)")
    return out
...
def record_simulation_execution(decision, action, entry_price, quantity, position=0.0,
    ...
    ex = Execution(
        ...
        # Baseline binding
        code_commit=decision.get('code_version', ''),
        policy_version=decision.get('config_version', ''),
        config_version=decision.get('config_version', ''),
        decision_cycle_version='daily_close',
    )
        ...
        if ex.get('decision_snapshot_id'):
            ...
            if snap_path.exists():
                try:
                    ...
                    print(f"[EXC] execution.py: {type(_e).__name__}: {_e}")
                    decision_snap = None
        if decision_snap:
            if not entry_regime:
...
def _compute_mae_mfe_from_klines(symbol, actual_entry_price, entry_time, exit_time):
    ...
    try:
        import sqlite3
        from pathlib import Path
        # 2026-09-19: 原硬编码 parent×3 解析到 scripts/skills/...（不存在）→ MAE/MFE 永远静默 None。
        # DB 规范路径由 core.compat_paths 单点解析（profile isolation 迁移后的唯一真相）。
        try:
            from core.compat_paths import get_market_cache_db
            db_path = Path(get_market_cache_db())
        except Exception:
            db_path = Path(__file__).resolve().parent.parent.parent.parent / 'stock-work' / 'data' / 'production' / 'market_cache.db'
        if not db_path.exists():
            return None
        from core.db_connection import connect_db
        con = connect_db(db_path, writer=True)
        cur = con.cursor()
