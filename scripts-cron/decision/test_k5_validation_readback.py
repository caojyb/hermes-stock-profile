...
class TestValidationTradeFilter(unittest.TestCase):
    ...
    def test_legacy_excluded(self):
        ...
        # 2026-09-19: 09-18 模拟仓重置（100 万基线）后 trades=0, 旧 32 笔已归档
        # trades_legacy_202607 表。validation window 无交易 → val=0; legacy 口径为
        # "trades 表中 <VALIDATION_START 的行" = 0（归档表不计入, 由迁移设计保证）。
        self.assertEqual(sim['validation_trades'], 0)
        self.assertEqual(sim['legacy_trades'], 0)
        # 归档表仍在（数据安全网）
        import sqlite3 as _s
        from core.compat_paths import SIMULATION_DB
        con = _s.connect(f'file:{SIMULATION_DB}?mode=ro', uri=True)
        n = con.execute('SELECT COUNT(*) FROM trades_legacy_202607').fetchone()[0]
        con.close()
        self.assertGreater(n, 0, 'legacy 归档表必须保留')
...
class TestNoWriteToProductionFacts(unittest.TestCase):
    ...
    def test_simulation_db_untouched(self):
        ...
        # （compat_paths 单点解析）; build_readback 必须只读不写
        from core.compat_paths import SIMULATION_DB
        before = Path(SIMULATION_DB).stat().st_mtime
        vr.build_readback('2026-08-27')
        after = Path(SIMULATION_DB).stat().st_mtime
