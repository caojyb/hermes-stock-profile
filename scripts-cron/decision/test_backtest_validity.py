...
# 2026-09-19: 旧硬编码路径指向迁移前 production/data/（DB 规范路径=data/production/）
try:
    from core.compat_paths import get_market_cache_db
    MKT_DB = get_market_cache_db()
except Exception:
    MKT_DB = os.path.join(os.environ.get('STOCK_WORK_ROOT', '/home/caojy/.hermes/profiles/stock/stock-work'),
                          'data', 'production', 'market_cache.db')
