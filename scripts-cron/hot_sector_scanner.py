...
OUTPUT_DIR = Path(__file__).resolve().parent.parent.parent / 'stock-work' / 'data' / 'outputs' / 'hot_sectors'
...
def fetch_industry_stocks(industry_name: str) -> list[dict]:
    ...
    for code, name in rows:
        cur2 = sqlite3.connect(MKT_DB, timeout=60)
...
def main():
    ...
    try:
        ...
        print(f"[WARN] 飞书发送失败: {e}", file=sys.stderr)


    # 心跳：记录本次扫描完成
    try:
        from heartbeat import write
        _results_count = len(results) if 'results' in locals() or 'results' in globals() else 0
        write(
            'hot-sector-scanner',
            'ok',
            detail=f'completed|{_results_count} sectors',
