...
def classify_data_health(*, timing_ok, kline_lag_days, signal_lag_days=None,
                         ...
                         north_coverage_ratio=None,
                         main_fund_coverage_ratio=None,
                         valuation_coverage_ratio=None,
                         financial_staleness_days=None):
    """从采集的原始指标归类数据健康等级。
    ...
    """
    coverage_warnings = []
    # 北向资金覆盖不足 → DEGRADED（候选池覆盖 < 50%）
    if north_coverage_ratio is not None and north_coverage_ratio < 0.5:
        coverage_warnings.append('NORTH_COVERAGE_LOW')
    # 主力资金覆盖不足 → DEGRADED（候选池覆盖 < 50%）
    if main_fund_coverage_ratio is not None and main_fund_coverage_ratio < 0.5:
        coverage_warnings.append('MAIN_FUND_COVERAGE_LOW')
    # 估值覆盖不足 → DEGRADED（候选池覆盖 < 50%）
    if valuation_coverage_ratio is not None and valuation_coverage_ratio < 0.5:
        coverage_warnings.append('VALUATION_COVERAGE_LOW')
    # 财务数据过期（> 180 天）→ DEGRADED
    if financial_staleness_days is not None and financial_staleness_days > 180:
        coverage_warnings.append('FINANCIAL_STALE')
...
def evaluate(*, regime_label=None, regime_scale=None,
    ...
    if critical_exit or data_health in (INVALID, MISSING):
        ...
        return _finalize(perm, status, reasons, 'SYSTEM_CRITICAL', position_scale)
    ...
    if data_health == STALE:
        ...
        perm['new_entry'] = DENY
        perm['add_position'] = DENY
        return _finalize(perm, status, reasons, 'NO_NEW_ENTRY', position_scale)
    ...
    if drawdown is not None and drawdown_limit and drawdown >= drawdown_limit:
        ...
        return _finalize(perm, status, reasons, 'REDUCE', position_scale)
    ...
    if regime in ('高波动', '低量能'):
        ...
        # 高波动/低量能：允许观察和小仓试探，但限制仓位缩放
        status = STATUS_REDUCE
        perm['new_entry'] = ALLOW
        perm['add_position'] = DENY
        if regime == '高波动':
            position_scale = min(position_scale, 0.15)
        else:
            position_scale = min(position_scale, 0.10)
    elif regime == '震荡市':
        ...
        status = STATUS_ALLOW
        position_scale = min(position_scale, 1.00)
    ...
    return _finalize(perm, status, reasons, status if status in _PRIORITY_ORDER else 'ALLOW', position_scale)


def _finalize(perm, status, reasons, priority, position_scale=1.0):
    return {
        'permission': perm,
        'status': status,
