#!/usr/bin/env python3
"""
c4_b2_stage3_full_matrix.py — M9.1-C4-B2-L Stage 3 evidence generation.
Uses canonical 8-variant matrix from c4_b2_full_matrix_runner.py.
Chunked by strategy family. Read-only. No production mutation.
"""
...
# Canonical 14 variants from D7-C4-B expansion registry
VARIANTS = [
...
def main():
    t0 = time.time()
    print(f"Stage 3: {len(VARIANTS)} variants × {FOLDS_COUNT} folds × {len(HORIZONS)} horizons = {len(VARIANTS) * FOLDS_COUNT * len(HORIZONS)} experiments", flush=True)

    variant_summaries = []
    family_summaries: Dict[str, List[Dict[str, Any]]] = {}
    ...
    for idx in range(len(VARIANTS)):
        ...
        family_summaries.setdefault(family, []).append(variant_summaries[-1])

    # Build full matrix
    all_experiments = []
    for idx in range(len(VARIANTS)):
        chunk_file = ARTIFACT_DIR / f"c4_b2_stage3_chunk_{idx:02d}_{VARIANTS[idx][0]}.json"
        chunk = json.loads(chunk_file.read_text(encoding="utf-8"))
        all_experiments.extend(chunk["experiments"])

    summary = {
        "total_variants": len(VARIANTS),
        "total_folds": FOLDS_COUNT,
        "total_horizons": len(HORIZONS),
        "total_experiments": len(VARIANTS) * FOLDS_COUNT * len(HORIZONS),
        "completed_experiments": len(all_experiments),
        "valid_experiment_count": sum(1 for e in all_experiments if e.get("fold_status") == "VALID_FOLD"),
        "failed_experiment_count": sum(1 for e in all_experiments if e.get("fold_status") not in ("VALID_FOLD", "LOW_SAMPLE")),
        "data_failure_count": sum(1 for e in all_experiments if e.get("fold_status") == "DATA_FAILURE"),
        "engine_failure_count": sum(1 for e in all_experiments if e.get("fold_status") == "ENGINE_FAILURE"),
        "pit_failure_count": sum(1 for e in all_experiments if e.get("fold_status") == "PIT_FAILURE"),
        "target_failure_count": sum(1 for e in all_experiments if e.get("fold_status") == "TARGET_FAILURE"),
        "reference_failure_count": sum(1 for e in all_experiments if e.get("fold_status") == "REFERENCE_FAILURE"),
        "low_sample_fold_count": sum(1 for e in all_experiments if e.get("fold_status") == "LOW_SAMPLE"),
        "cache_hit_count": sum(s["cache_hits"] for s in variant_summaries),
        "cache_miss_count": sum(s["cache_misses"] for s in variant_summaries),
        "elapsed_seconds": round(time.time() - t0, 3),
    }
    ...
    (ARTIFACT_DIR / "c4_b2_stage3_full_matrix.json").write_text(
        json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    (ARTIFACT_DIR / "c4_b2_stage3_execution_metrics.json").write_text(
