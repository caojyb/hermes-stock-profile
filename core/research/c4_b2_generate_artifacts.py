#!/usr/bin/env python3
"""
Generate C4-B2-G artifacts:
1. c4_b2_fold_boundary_before_after.json
2. c4_b2_stage2_trend_8fold_aligned.json
3. M9_1_C4_B2_G_TRADING_CALENDAR_FOLD_ALIGNMENT.md
"""
...
# Build before/after from audit raw + aligned results
before_after = {
...
# Copy aligned results to canonical aligned filename
...
for f in before_after['folds']:
    report.append(f"| {f['fold_id']} | {f['fold_status_before']} | {f['fold_status_after']} | {f['valid_target_count_before'] or 'N/A'} | {f['valid_target_count_after']} |")
report.append("")
...
for f in nf:
    ...
    aligned_start = f['aligned_validation_start']
    report.append(f"| {raw} | {'Sunday' if raw in ['2025-07-27','2026-04-25'] else 'Holiday'} | {aligned_start} | {f['fold_status_after']} ({f['valid_target_count_after']} targets) |")
report.append("")
