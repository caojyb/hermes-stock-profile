#!/usr/bin/env python3
"""
c4_b2_full_matrix_trace3.py — instrument runner for a single fold.
Logs: universe size, raw signals, eligible count, and target engine outputs.
Read-only.
"""
...
def main():
    ...
    print("fold:", fold.fold_id, fold.validation_start, fold.validation_end, "universe:", len(universe), flush=True)
    ...
    print("strategy kline_loader:", strategy.kline_loader is not None, flush=True)
    print("baseline kline_loader:", baseline.kline_loader is not None, flush=True)

    # Try direct strategy run to inspect raw signals/candidates
    print("running strategy.run", flush=True)
    strategy_out = strategy.run(universe, fold.validation_start)
    print("strategy.run type:", type(strategy_out), flush=True)
    if isinstance(strategy_out, dict):
        print("strategy.run keys:", sorted(strategy_out.keys()), flush=True)
        print("signals:", strategy_out.get("signal_count"), flush=True)
        print("candidates:", len(strategy_out.get("candidates", [])), flush=True)
        print("sample candidates:", strategy_out.get("candidates", [])[:3], flush=True)
    else:
        print("strategy.run result:", strategy_out, flush=True)

    # Now run full fold path
    print("starting engine.run_fold", flush=True)
    try:
